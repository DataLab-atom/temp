...
class Acausation_sub_encoder(torch.nn.Module):
    def __init__(self, args,att_cov,gfn=False,edge_norm=True):
        super(Acausation_sub_encoder, self).__init__()
        self.args = args
...
class Causal_sub_encoder(torch.nn.Module):
    def __init__(self,args,att_cov,gfn=False,edge_norm=True):
        super(Causal_sub_encoder, self).__init__()
        self.args = args
...
class Acausation_Classifier(torch.nn.Module):
    def __init__(self, num_classes, args):
        super(Acausation_Classifier, self).__init__()
...
class Causal_Classifier(torch.nn.Module):
    def __init__(self, num_classes, args):
...
class Causal_Interventions_Classifier(torch.nn.Module):
    def __init__(self, num_classes, args):
        super(Causal_Interventions_Classifier, self).__init__()
...
class CausalGCN(torch.nn.Module):
    ...
    def __init__(self, num_features,
        ...
        self.Gnn_encoder = GNN_encoder(num_features,args,bns_conv,convs)
        self.Acausation_sub_encoder = Acausation_sub_encoder(args,attcov,gfn,edge_norm)
        self.Causal_sub_encoder = Causal_sub_encoder(args,attcov,gfn,edge_norm)
        self.Acausation_Classifier = Acausation_Classifier(num_classes,args)
        self.Causal_Classifier = Causal_Classifier(num_classes,args)
        self.Causal_Interventions_Classifier = Causal_Interventions_Classifier(num_classes,args)

    def forward(self, data, eval_random=True):
        x = data.x if data.x is not None else data.feat
        edge_index, batch = data.edge_index, data.batch
        
        x = self.Gnn_encoder(x,edge_index,False)
        xc,_,_ = self.Acausation_sub_encoder(x,edge_index,batch,eval_random)
        xo,_,_ = self.Causal_sub_encoder(x,edge_index,batch,eval_random)

        xc_logis = self.Acausation_Classifier(xc)
        xo_logis = self.Causal_Classifier(xo)
        xco_logis = self.Causal_Interventions_Classifier(xc,xo,eval_random)
